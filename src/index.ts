...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Chile Government Procurement');
}
...
async function searchTenders(args: Record<string, unknown>, ticket: string): Promise<unknown> {
  ...
  // A keyword with no explicit window means "find me tenders about X", which is
  // a question about the OPEN set, not about the ~20 notices published today.
  // Default accordingly: a router that filled {fecha:"today", keyword:"camion"}
  // searched 23 notices, matched 0, and produced a confident "there are no truck
  // tenders in Chile" — wrong, and the tool made it easy to be wrong.
  const keywordOnly = !!strArg(args.keyword) && !fecha && !strArg(args.estado);
  const estado = keywordOnly ? 'activas' : strArg(args.estado);
  ...
  const matched = listado.length;

  // Keyword filter. ChileCompra's endpoint has NO text search — it only accepts
  // fecha/estado/codigo — so a question like "tenders for trucks" previously
  // returned the first 25 of ~4,300 active notices sorted by código, i.e. noise
  // presented as an answer. The API hands back the whole active list in one
  // response, so filtering here before the slice is both correct and free.
  // (Same shape as the canada-tenders all-words fix.)
  const keyword = strArg(args.keyword);
  if (keyword) {
    // Strip accents + lowercase so "camion" matches "CAMIÓN"/"CAMIONES".
    const norm = (s: string) => s.normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase();
    // Spanish plural → stem. Substring matching alone is asymmetric: "camion"
    // finds CAMIONES, but "camiones" does NOT find CAMIÓN. The router picks
    // either form run to run, so the same question returned 38 matches or 9
    // depending on which it happened to choose. Stemming makes both agree —
    // and makes the description's "matches word stems" claim true, which it
    // wasn't before.
    const stem = (w: string) =>
      w.length > 4 && w.endsWith('es') ? w.slice(0, -2)
      : w.length > 4 && w.endsWith('s') ? w.slice(0, -1)
      : w;
    const words = norm(keyword).split(/\s+/).filter(Boolean).map(stem);
    listado = listado.filter((t) => {
      const hay = norm(String(t?.Nombre ?? ''));
  ...
  const tenders = listado.slice(0, limit).map((t) => ({
    ...
    // Deep link to the notice on Mercado Público. Verified 2026-07-25: this
    // form resolves to the specific tender (page carries the código + name).
    // Without it a caller has a list of códigos and no way to act on any of them.
    url: t.CodigoExterno
      ? `https://www.mercadopublico.cl/Procurement/Modules/RFB/DetailsAcquisition.aspx?idlicitacion=${encodeURIComponent(t.CodigoExterno)}`
      : null,
    ...(t.Comprador?.NombreOrganismo ? { buyer: t.Comprador.NombreOrganismo } : {}),
  }));
  // One unambiguous sentence stating what the numbers mean. Three bare counts
  // (total / matching_keyword / returned) invite a reader — human or model — to
  // quote the wrong one: live synthesis said "25 active truck tenders" on 2 of 5
  // runs when the true match count was 38 and 25 was merely the page size.
  const resultSummary = keyword
    ? `${listado.length} of ${data.Cantidad ?? listado.length} ${fecha ? `tenders published ${fecha}` : 'active tenders'} match "${keyword}"; showing the first ${Math.min(limit, listado.length)}.`
    : `${data.Cantidad ?? tenders.length} ${fecha ? `tenders published ${fecha}` : 'active tenders'}; showing the first ${tenders.length}.`;

  return {
    result_summary: resultSummary,
    filter: {
      ...(fecha ? { fecha } : estado ? { estado } : { fecha: 'today' }),
      ...(keyword ? { keyword } : {}),
    },
    total: data.Cantidad ?? tenders.length,
    // With a keyword, `total` is the unfiltered API count — reporting only that
    // would imply thousands of matches. Say how many actually matched.
    ...(keyword ? { matching_keyword: listado.length, searched: matched } : {}),
    returned: tenders.length,
    ...(keyword && listado.length === 0
      ? { note: `No active Chile tenders currently match "${keyword}". Tender names are in Spanish — try a Spanish term (e.g. "camion" for trucks).` }
      : {}),
    tenders,
  };
}
